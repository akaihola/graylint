...
class PylintParserPlugin(GnuErrorFormatParserPlugin):
    ...
    def parse(
        ...
        :param cwd: The root directory to use for relative paths
    ...
    def parse_lines(
        ...
        """Parse the list of output lines from Pylint.

        :param linter: The name of the linter
        :param lines: An iterator of lines of Pylint output, including trailing newlines
        :param cwd: The root directory to use for relative paths
        :return: A generator of tuples of message locations and linter messages
    ...
    def parse_r0801(
        ...
        linter: str,
        lines: Iterator[str],
        cwd: Path,  # pylint: disable=unused-argument
        match: Match[str],
    ) -> Iterator[tuple[MessageLocation, LinterMessage]]:
        """Parse a "Similar lines in N files" message.

        :param linter: The name of the linter
        :param lines: An iterator over the lines of the output
        :param cwd: The root directory to use for relative paths
        :param match: The match object for the "Similar lines in N files" message
        :return: An iterator over the parsed messages and their locations

        """
        num_duplicates = int(match.group(1))
        num_lines = 0
        for line in islice(lines, num_duplicates):
            ...
            if num_lines == 0:
                ...
                num_lines = min(num_lines, num_lines_here)
        for _ in islice(lines, num_lines):
            pass
